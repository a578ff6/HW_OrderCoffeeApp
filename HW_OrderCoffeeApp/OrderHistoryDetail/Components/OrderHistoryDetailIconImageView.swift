//
//  OrderHistoryDetailIconImageView.swift
//  HW_OrderCoffeeApp
//
...
/// `OrderHistoryDetailIconImageView` 是一個通用的自訂 `UIImageView`，
/// 用於展示歷史訂單詳細頁面中的圖示（如數量圖示或其他圖標）。
///
/// 此類別支援以下功能：
/// - 預設支援 SF Symbols，並可自定義圖示大小與顏色。
/// - 提供靈活的樣式設置，適用於多個元件中。
/// - 預設適配 AutoLayout，無需額外設定。
class OrderHistoryDetailIconImageView: UIImageView {
