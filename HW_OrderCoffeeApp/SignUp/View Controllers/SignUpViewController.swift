...
` 1.初始化 SignUpView：`
 ...
 - 使用者點擊「閱讀條款」按鈕後，開啟條款的連結，如果成功開啟，則通過 signUpView.enableTermsCheckBox() 啟用「同意條款」的勾選框。
...
/**
 ...
 `1. 提取用戶輸入：`
    - 將 `signUpView.getFullName()`、`getEmail()`、`getPassword()` 等方法調用集中到方法的最初始階段，減少重複調用。
...
extension SignUpViewController: SignUpViewDelegate {
    ...
    func signUpViewDidTapTermsButton() {
        if let url = URL(string: "https://www.starbucks.com.tw/stores/allevent/show.jspx?n=1016") {
            UIApplication.shared.open(url) { [weak self] success in
                if success {
                    self?.signUpView.enableTermsCheckBox()
                }
            }
        }
    }
    ...
    func signUpViewDidTapTermsCheckBox() {
        ...
        signUpView.toggleTermsCheckBoxSelection()
    }
    ...
    func signUpViewDidTapSignUpButton() {
        ...
        let fullName = signUpView.getFullName()
        let email = signUpView.getEmail()
        let password = signUpView.getPassword()
        let confirmPassword = signUpView.getConfirmPassword()
    ...
    func signUpViewDidTapAppleSignUpButton() {
        print("signUpViewDidTapSignUpButton")
        Task {
            HUDManager.shared.showLoading(text: "Logging in...")
            do {
