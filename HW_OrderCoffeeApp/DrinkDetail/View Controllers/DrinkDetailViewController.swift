...
// MARK: - drinkID & asyc & 分享、我的最愛已經完善 & 當 HUD 顯示時，不需要手動控制按鈕的禁用與否，因為 HUD 已經會禁用所有的互動。
/*
import UIKit
...
class DrinkDetailViewController: UIViewController {
    ...
    private func handleDrinkLoaded(_ drink: Drink) {
        ...
        updateNavigationButtonsAppearance()    // 當資料載入完成後，更新按鈕的視覺狀態即可，無需再管理按鈕的互動性。
    }
    ...
    private func setupNavigationBarItems() {
        ...
        updateNavigationButtonsAppearance()                                   // 當資料加載完成後更新按鈕的視覺狀態
    }
    
    /// 更新 `NavigationButtons` 按鈕的視覺狀態
    private func updateNavigationButtonsAppearance() {
