...
/*
 ...
 3. 哪一種更合適：
    ...
    - 如果想要更多的 UI 控件和靈活性（如在地圖之上還要顯示店鋪列表、距離顯示或選擇按鈕），建議使用 StoreSelectionView: UIView，然後在其中嵌入一個 MKMapView。這樣可以根據需要自由設置不同 UI 元素的位置和樣式。
...
/// 用於顯示店家選擇的自訂簡視圖
/// StoreSelectionView 包含一個 MKMapView 來幫助用戶選擇近部的店家
class StoreSelectionView: UIView {
    
    // MARK: - UI Elements
    
    let mapView = StoreSelectionView.createMapView()
