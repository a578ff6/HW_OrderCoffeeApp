...
/// `OrderConfirmationLabel` 是一個自訂的 `UILabel`，
/// 專門用於 `OrderConfirmation` 中的標籤樣式設定。
///
/// 此類別支援以下功能：
/// - 自訂字體 (`font`) 與文字顏色 (`textColor`)。
/// - 設定行數限制 (`numberOfLines`) 及文字對齊方式 (`textAlignment`)。
/// - 自動縮放文字以適應視圖大小，並可透過 `scaleFactor` 設定最小縮放比例。
class OrderConfirmationLabel: UILabel {
    
    // MARK: - Initializer
    
    /// 初始化 `OrderConfirmationLabel`
    /// - Parameters:
