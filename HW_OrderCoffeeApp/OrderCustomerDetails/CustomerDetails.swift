...
/*
 ## 筆記：
    ...
    - pickupMethod 可以是到店取件或宅配運送，這個屬性影響了其他欄位是否需要填寫（如地址或取件店名）。
    - 如果顧客選擇宅配，就會需要填寫 address，而到店取件則需填寫 storeName。
 */
...
struct CustomerDetails: Codable {
    ...
    var pickupMethod: PickupMethod            // 取件方式，包含 "到店取件" 或 "宅配運送"
    var address: String?                      // 配送地址（選填），只有選擇宅配時才需填寫
...
enum PickupMethod: String, Codable {
    ...
    case homeDelivery = "Home Delivery"       // 宅配運送
}
